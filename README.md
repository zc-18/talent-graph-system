...
- **数据规模**：32 岗位（新兴 6）、3952 技能（粗 136 / 细 3816）、2570 条真实 JD，去重后有效 2404 条（重复 166）。
- **能力与证据**：active/candidate/deprecated = **812/3604/1231**；Evidence **9128**（均带 RawJD）；AuthorityEvidence **21**，覆盖 **19/32 岗**，6/6 新兴岗位全覆盖且共挂 8 条。
...
- **置信与治理**：全库置信度 **0.5502**（2026-08-30 口径）（既有 0.5271 / 新兴 0.6507；R6 前基线 0.4783）。均值按约 -0.0003/天 自然衰减（时效性因子 + 每日 02:30 批算重跑），故一律带口径日期；这是设计特性，见《作品设计与实现方案》。雇主识别 **2105/2404（87.56%）**；RoleContract **6/32 ready（18.75%）**。
...
# 生产图谱是 talent_graph_v4。按发布门禁，构建/演化只在影子库上做，验收通过后才提升为发布库；
# 不先指定库名会静默写错库（见 docs/部署说明 第五节）：
$env:DB_NAME='talent_graph_v4_shadow'   # PowerShell（bash: export DB_NAME=talent_graph_v4_shadow）
