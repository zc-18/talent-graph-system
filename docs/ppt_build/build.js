...
const kpis = [['29', '岗位'], ['105', '技能点'], ['303', '岗位JD'], ['1439', '溯源证据'], ['92.7%', '平均置信度']]
kpis.forEach(([v, l], i) => {
  const x = 0.9 + i * 2.45
...
const clean = [
  ...
  [AMBER, '能力通胀', '技能数远超中位数 + 非共识技能占比高', '13/13 通胀样本检出，噪声技能被过滤'],
...
// ============ 10. 测试与指标 ============
s = p.addSlide(); base(s); header(s, '09', '测试与验证', '完整测试方案（303 条 JD）· 三项核心指标全部超过 90%')
const metrics = [
  ['98.6%', 'JD 解析准确率', 'F1 · 295 条非重复 JD', EMERALD],
...
const extra = [['8 / 8', '抄袭检出'], ['4 / 4', '通胀检出'], ['64%', '单元测试覆盖率'], ['44', '测试用例全通过']]
extra.forEach(([v, l], i) => {
  const x = 0.6 + i * 3.1
...
s.addText('数据驱动 + 大模型 + 知识图谱  ·  多源交叉验证  ·  反幻觉防控  ·  动态演化', { x: 0.55, y: 5.4, w: 12.2, h: 0.4, align: 'center', fontFace: F, fontSize: 14, color: 'DBEAFE' })
