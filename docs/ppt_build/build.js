...
const kpis = [['29', '岗位（新兴6）'], ['2387', '技能（含1961技能点）'], ['806', '真实岗位JD'], ['3363', '溯源证据'], ['0.535', '岗位置信度均值']]
kpis.forEach(([v, l], i) => {
  const x = 0.9 + i * 2.45
...
// ============ 3.5 真实数据采集与合规 ============
s = p.addSlide(); base(s); header(s, '02+', '真实数据采集与合规', '806 条真实 JD · 官网公开接口合法采集 · 全程台账可溯源')
const srcs = [
...
s.addText('数据源台账在系统首页可视化：采集 806 → 查重 → 解析 → 交叉验证 → 入图谱，闭环每一环都有真实数字可查；合成数据集（379 条）重新定位为对抗测试夹具，不进生产图谱', {
...
// ============ 6.5 分级演化：初/中/高晋升路径 ============
s = p.addSlide(); base(s); header(s, '05+', '分级演化 · 初/中/高晋升路径', '22 个岗位建有三档分级画像 · 14 个岗位完成 2018→2026 真实八年演化（100 条记录）')
const lvls = [
...
s.addText('岗位整体置信度 = 粗粒度 active 能力项的权重加权平均；全库岗位均值 0.535 —— 真实数据下的诚实数字（1021 项低置信 candidate 被过滤，不进正式图谱）', {
  x: 0.55, y: 6.2, w: 12.2, h: 0.6, align: 'center', valign: 'middle', fontFace: F, fontSize: 12, bold: true, color: INDIGO,
  fill: { type: 'solid', color: 'E0E9FF' }, line: { type: 'none' } })
footer(s, 10)

// ============ 10. 测试与指标 ============
s = p.addSlide(); base(s); header(s, '09', '测试与验证', '双轨数据集（对抗基准 379 条 + 真实语料 806 条）· 三项核心指标全部超过 90%')
const metrics = [
