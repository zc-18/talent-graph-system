      ...
      {!loading && classification === 'NEW' && definition && (
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-5 items-start">
          <Card className="p-6">
            ...
            <div className="label mt-5 mb-2">核心职责</div>{editing ? <textarea value={(definition.core_responsibilities || []).join('\n')} onChange={event => setDefinition({ ...definition, core_responsibilities: event.target.value.split('\n').filter(Boolean) })} rows={6} className="input resize-none" /> : <ol className="space-y-2">{(definition.core_responsibilities || []).map((item: string, index: number) => <li key={index} className="flex gap-2 text-sm text-body-2"><b className="text-accent">{index + 1}</b>{item}</li>)}</ol>}
