...
export default function Discovery() {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading && <Card className="p-10 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-accent" /><p className="text-sm text-slate-500 mt-3">正在进行成熟岗位否决、历史新颖性与多雇主证据核验…</p><p className="text-xs text-slate-400 mt-1.5">任务在后台执行，切到其他页面再回来仍可看到结果</p></Card>}
      ...
      {!loading && classification === 'NEW' && definition && (
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-5 items-start">
          ...
          <div className="space-y-4"><Card className="p-5"><div className="label mb-3">新兴度判定信号</div><div className="space-y-4">{signals.map(signal => <div key={signal.key}><div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="font-medium text-slate-600">{signal.label}</span><b className={signal.value == null ? 'font-medium text-slate-400' : 'tabular-nums text-slate-800'}>{signal.value == null ? (signal.empty || '暂无数据') : `${Math.round(signal.value * 100)}%`}</b></div><Meter value={signal.value || 0} min={signal.value == null ? 0 : 3} tone={signal.value == null ? 'bg-slate-200' : 'bg-accent-deep'} track="bg-slate-100" /></div>)}</div></Card><Card className="p-5"><div className="flex items-center gap-2 label mb-3"><IGlobe className="w-4 h-4 text-accent-deep" /> 可追溯证据 ({runData?.evidence?.length || 0})</div><div className="space-y-2 max-h-[460px] overflow-auto">{(runData?.evidence || []).map((item: any, index: number) => <a key={index} href={item.url} target="_blank" rel="noreferrer" className="block rounded-xl bg-sky-50/70 hover:bg-sky-100 px-3 py-2.5"><div className="flex items-center justify-between gap-2"><span className="text-xs font-medium text-slate-700 truncate">{item.title || item.job_title || '证据来源'}</span><ExternalLink className="w-3 h-3 text-slate-400 shrink-0" /></div><p className="text-[11px] text-slate-400 mt-1 line-clamp-2">{item.content || item.snippet}</p><span className="text-[10px] text-accent">{item.company || item.employer || item.provider || item.source}</span></a>)}</div></Card></div>
        </div>
      )}
    </div>
  )
