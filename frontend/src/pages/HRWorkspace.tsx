...
function SectionError({ message }: { message: string }) {
  return (
    <div className="rounded-lg bg-danger-weak border border-danger/20 px-3 py-2 text-xs text-danger">
      {message}
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3"><div className="w-11 h-11 rounded-xl bg-grad-accent grid place-items-center shadow-glow"><BriefcaseBusiness className="w-5 h-5 text-white" /></div><div><h1 className="text-2xl font-extrabold text-body-1">HR 招聘工作台</h1><p className="text-sm text-body-2">批量解析、统一岗位契约排名、Top-K 入团队</p></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[360px_minmax(0,1fr)] gap-5 items-start">
        <div className="space-y-4">
          <Card className="p-5 space-y-3">
            ...
            <label className="block"><span className="text-xs text-body-2">保留天数</span><input type="number" min={1} max={180} value={retentionDays} onChange={e => setRetentionDays(Number(e.target.value))} className="input mt-1" /></label>
          ...
          <Card className="p-5 space-y-3">
            ...
            {current ? <><div className="flex items-center justify-between text-xs"><Badge tone={current.status === 'completed' ? 'emerald' : current.status === 'completed_with_errors' ? 'amber' : current.status === 'failed' ? 'rose' : 'cyan'}>{current.status}</Badge><span className="text-body-2 tabular-nums">{progress.processed}/{progress.total}</span></div><Meter value={progressRate} /><div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg bg-accent/8 p-2"><b className="block text-accent-deep">{progress.succeeded}</b>成功</div><div className="rounded-lg bg-danger-weak p-2"><b className="block text-danger">{progress.failed}</b>失败</div><div className="rounded-lg bg-accent/6 p-2"><b className="block text-accent-deep">{progress.total - progress.processed}</b>待处理</div></div></> : <EmptyState text="暂无批次" />}
            <label className="flex items-start gap-2 text-xs text-body-2 leading-relaxed"><input type="checkbox" checked={authorized} onChange={e => setAuthorized(e.target.checked)} className="mt-0.5 accent-accent" /><span>我确认已获得候选人简历的处理授权，结构化画像将按保留期自动清理。</span></label>
        ...
        <Card className="p-0 overflow-hidden">
          <div className="p-5 border-b border-line-soft/8 flex items-center justify-between gap-3 flex-wrap">
            <div><div className="font-bold text-body-1">候选人排名</div><div className="text-xs text-body-3 mt-0.5">历史排名固定引用批次创建时的岗位版本</div></div>
          ...
          {rankingError ? <div className="p-6"><ErrorState text={rankingError} onRetry={() => batchId && void loadRanking(batchId)} /></div>
            ...
            <div className="overflow-x-auto"><table className="w-full text-sm min-w-[640px]"><thead className="text-xs text-body-2 bg-surface-muted"><tr><th className="p-3 text-left w-12">选择</th><th className="p-3 text-left">排名/候选人</th><th className="p-3 text-left">综合分</th><th className="p-3 text-left">维度</th><th className="p-3 text-left">状态</th></tr></thead><tbody>{ranking.map(row => <tr key={row.candidate_id} className="border-t border-line-soft/6 hover:bg-accent/4"><td className="p-3"><button aria-label={`选择 ${row.code}`} onClick={() => setSelected(prev => { const next = new Set(prev); next.has(row.candidate_id) ? next.delete(row.candidate_id) : next.add(row.candidate_id); return next })} className={`w-5 h-5 rounded border grid place-items-center ${selected.has(row.candidate_id) ? 'bg-accent text-white border-accent' : 'border-line-soft/14'}`}>{selected.has(row.candidate_id) && <Check className="w-3.5 h-3.5" />}</button></td><td className="p-3"><b className="text-body-1 mr-2">#{row.rank}</b><span className="text-body-2">{row.code}</span></td><td className="p-3 font-extrabold text-lg text-body-1">{score(row.overall_score)}</td><td className="p-3"><div className="flex flex-wrap gap-1">{Object.entries(row.dimension_scores || {}).slice(0, 4).map(([key, value]) => <Badge key={key} tone="slate">{key} {score(value)}</Badge>)}</div></td><td className="p-3"><Badge tone="cyan">{row.status}</Badge></td></tr>)}</tbody></table></div>
          )}
          <div className="p-5 border-t border-line-soft/8 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3"><div className="text-sm text-body-2">{errors.teams ? <span className="text-danger">{errors.teams}</span> : <>已选 <b className="text-body-1">{selectedRows.length}</b> 人</>}</div><div className="flex flex-col sm:flex-row gap-2"><Select value={teamId ?? ''} onChange={value => setTeamId(value ? Number(value) : null)} placeholder="新建 Top-K 团队" className="sm:w-52" options={[{ value: '', label: '新建 Top-K 团队' }, ...teams.map(team => ({ value: String(team.id), label: team.name }))]} /><button onClick={commitSelection} disabled={busy || selected.size === 0} className="btn-primary justify-center"><UsersRound className="w-4 h-4" /> {teamId ? '加入团队' : '创建团队并入组'}</button></div></div>
          {current?.failures && current.failures.length > 0 && <div className="p-5 border-t border-danger/20 bg-danger-weak/60"><div className="flex items-center gap-2 text-sm font-semibold text-danger"><FileArchive className="w-4 h-4" /> 失败明细</div><div className="text-xs text-danger mt-2 space-y-1">{current.failures.slice(0, 10).map((failure: any, index) => <div key={index}>{failure.filename || failure.code || `文件 ${index + 1}`} · {failure.detail || failure.reason}</div>)}</div></div>}
        </Card>
      </div>
    </div>
  )
